...
class PropertyController extends Controller
{
    ...
    { 
        ...
        if(isset($image)){
            ...
            // Determine safe format from MIME type
            ...
            $propertyimage = Image::make($image)->encode($format, 90);
        ...
        if($gallary)
        {
            foreach($gallary as $images)
            {
                $currentDate = Carbon::now()->toDateString();
                // Determine safe format from MIME type
                $mimeG = $images->getMimeType();
                $formatG = strtolower(explode('/', $mimeG)[1] ?? 'jpg');
                if ($formatG === 'jpeg') { $formatG = 'jpg'; }
                if (!in_array($formatG, ['jpg','png','gif','webp'])) { $formatG = 'jpg'; }
                $galimage['name'] = 'gallary-'.$currentDate.'-'.uniqid().'.'.$formatG;
                ...
                $propertyimage = Image::make($images)->encode($formatG, 90);
