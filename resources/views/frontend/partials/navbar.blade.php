<div class="navbar-fixed">
    <nav class="nav-modern nav-green">
        <div class="container">
            <div class="nav-wrapper">

                <a href="{{ route('home') }}" class="brand-logo brand-logo-img">
                    <img src="{{ asset('frontend/images/eduflare-logo.png') }}" alt="Eduflare" class="logo-img" onerror="this.style.display='none'; if(this.nextElementSibling){ this.nextElementSibling.style.display='inline-block'; }">
                    <span class="logo-text" style="display:none;">Eduflare</span>
                </a>
                    ...
                    <li class="hide-on-small-only">
                        @auth
                            @if(Auth::user()->role && (in_array(Auth::user()->role->id, [1,2])))
                                <a href="{{ Auth::user()->role->id == 1 ? route('admin.dashboard') : route('agent.dashboard') }}" class="btn-small waves-effect btn-cta">List a Property</a>
                            @else
                                <a href="{{ route('login') }}" class="btn-small waves-effect btn-cta">List a Property</a>
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="btn-small waves-effect btn-cta">List a Property</a>
                        @endauth
