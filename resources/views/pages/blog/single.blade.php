                        ...
                        <div class="single-narebay p-15">
                            ...
                            @foreach($post->comments as $comment)
                                @if($comment->parent_id == null)
                                    <div class="comment">
                                        @php $commentImage = optional($comment->user)->image ?? 'default.png'; @endphp
                                        <div class="author-image">
                                            <span style="background-image:url('{{ Storage::url('users/'.$commentImage) }}');"></span>
                                        </div>
                                        <div class="content">
                                            <div class="author-name">
                                                <strong>{{ optional($comment->user)->name ?? 'Unknown User' }}</strong>
                                            ...
                                            <div class="replies">
                                                @foreach($comment->children as $reply)
                                                    <div class="comment reply">
                                                        @php $replyImage = optional($reply->user)->image ?? 'default.png'; @endphp
                                                        <div class="author-image">
                                                            <span style="background-image:url('{{ Storage::url('users/'.$replyImage) }}');"></span>
                                                        </div>
                                                        <div class="content">
                                                            <div class="author-name">
                                                                <strong>{{ optional($reply->user)->name ?? 'Unknown User' }}</strong>
